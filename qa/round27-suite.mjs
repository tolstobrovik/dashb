...
await fullForm()
await p.selectOption('.modal select[data-pick="type"]', 'video'); await p.waitForTimeout(400)
...
await p.waitForSelector('.modal', { timeout: 6000 })
await fullForm()
await p.selectOption('.modal select[data-pick="type"]', 'video'); await p.waitForTimeout(600)
